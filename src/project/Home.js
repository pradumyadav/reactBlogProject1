...
export default function Home(){
    ...
    return(
            ...
            .map((item,index)=>{
                return(
                    <div className="home_Header"key={index}>
                     ...
                     <img src={item.dataImage1} style={{width:"800px",height:"460px",marginRight:"65px",borderRadius:"20px"}}/>
                         <div className="home_Header2">
                       <img src={item.dataImage2} style={{width:"350px",height:"220px",marginBottom:"20px",borderRadius:"20px"}}/>
                       <img src={item.dataImage3} style={{width:"350px",height:"220px",borderRadius:"20px"}}/>
            ...
            <div className="the_Latest"><h2>The Latest</h2></div>
            ...
            .map((item,index)=>{
                return(
                   <div className="home_body"key={index}>
                     ...
                     <div>  <img className="home_body_img"src={item.dataImage} style={{width:"370px",borderRadius:"15px"}}/></div>
                ...
                <h2 className="home_H2">Top Stories</h2>
                <h2 className="home_top_post">Top Post</h2>

               <div className="home_flex2">
                <div>
                {homedata_from_roomData.filter((item)=>{return(item.ID===4||item.ID===18||item.ID===45||item.ID===51||item.ID===59||item.ID===60||item.ID===80)})
            .map((item,index)=>{
                return (
                <div className="childcompo"key={index}>
                    <NavLink to={`/ContentDeatails/${item.ID}`}>
                    <div className="titlediv">{item.dataTitle.slice(0,40)}...</div>
                 <div><img className="home_img" src={item.dataImage} style={{width:"300px"}}/> </div>
            ...
            {homedata_from_roomData.filter((item)=>item.ID ===29).map((item,index)=>{
                return(
                    <div key={index}>
                        ...
                        <div className="latest_post2"> {item.dataTitle.slice(0,70)}...<div className="one">{index+1}</div></div>
                         </NavLink>
                ...
                {homedata_from_roomData.filter((item)=>{return(item.ID===1||item.ID===16||item.ID===41||item.ID===51||item.ID===78)})
            .map((item,index)=>{
                return (
